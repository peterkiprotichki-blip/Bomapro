...
export class AuthService {
  ...
  login(email: string, password: string): Observable<AuthResponse> {
    return this.http.post<AuthResponse>(`${this.apiUrl}/login`, { email, password }).pipe(
      tap((res) => {
        localStorage.setItem('bomapro_token', res.token);
        localStorage.setItem('bomapro_user', JSON.stringify(res.user));
  ...
  handleGoogleCallback(token: string, user: BomoproUser, tenants: Tenant[] = [], activeTenantId: string = ''): void {
    localStorage.setItem('bomapro_token', token);
    localStorage.setItem('bomapro_user', JSON.stringify(user));
  ...
  logout(): void {
    localStorage.removeItem('bomapro_token');
    localStorage.removeItem('bomapro_user');
    localStorage.removeItem('bomapro_tenants');
    localStorage.removeItem('bomapro_active_tenant');
  ...
  getToken(): string | null {
    ...
    return localStorage.getItem('bomapro_token') || localStorage.getItem('rentium_token');
  }
  ...
  setTenantContext(tenants: Tenant[], activeTenantId = ''): void {
    ...
    if (normalizedTenants.length) {
      localStorage.setItem('bomapro_tenants', JSON.stringify(normalizedTenants));
    } else {
      localStorage.removeItem('bomapro_tenants');
    }

    if (resolvedActiveTenantId) {
      localStorage.setItem('bomapro_active_tenant', resolvedActiveTenantId);
    } else {
      localStorage.removeItem('bomapro_active_tenant');
    }
  ...
  private loadUser(): void {
    const userJson = localStorage.getItem('bomapro_user') || localStorage.getItem('rentium_user');
    if (userJson) {
      try { this.currentUser$.next(JSON.parse(userJson)); } catch (e) {}
    }
    const tenantsJson = localStorage.getItem('bomapro_tenants') || localStorage.getItem('rentium_tenants');
    let tenants: Tenant[] = [];
    if (tenantsJson) {
      try { tenants = JSON.parse(tenantsJson); } catch (e) {}
    }
    this.setTenantContext(tenants, localStorage.getItem('bomapro_active_tenant') || localStorage.getItem('rentium_active_tenant') || '');
  }
}
